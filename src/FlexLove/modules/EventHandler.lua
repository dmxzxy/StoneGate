...
function EventHandler:processTouchEvents(element)
  ...
  -- Get current active touches from L脰VE
  local activeTouches = {}
  ...
  for _, id in ipairs(touches) do
    ...
    local pressure = 1.0 -- L脰VE doesn't provide pressure by default
...
function EventHandler:_handleTouchEnded(element, touchId, x, y, pressure)
  ...
  -- Synthesize a "click" event for taps (small movement) so buttons using onEvent
  -- work on touch devices. Mirrors the mouse path which fires click on release.
  local moveDist = math.sqrt(dx * dx + dy * dy)
  local tapMaxMovement = 10 -- px, matches GestureRecognizer default
  if moveDist <= tapMaxMovement then
    local clickEvent = EventHandler._InputEvent.new({
      type = "click",
      button = 1,
      x = x,
