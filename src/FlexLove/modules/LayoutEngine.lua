...
function LayoutEngine:recalculateUnits(newViewportWidth, newViewportHeight)
  ...
  -- For any explicitly-sized element (viewport/percent/px), `width` is the
  -- CONTENT width (= border-box minus left+right padding). This keeps the
  -- flex wrap calculation correct: availableMainSize = self.element.width
  -- already excludes padding, so a row that fits exactly N cards will wrap
  -- on the (N+1)th as expected.
  if self.element.units.width.unit ~= "auto" then
    self.element.width =
  ...
  if self.element.units.height.unit ~= "auto" then
    self.element.height =
